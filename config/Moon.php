...
class MoonPhase
{
	...
	*/
	...
	public function phase_name_EN()
	{
		$nameEN = array('New Moon', 'Waxing Crescent', 'First Quarter', 'Waxing Gibbous', 'Full Moon', 'Waning Gibbous', 'Third Quarter', 'Waning Crescent', 'New Moon');

		return $nameEN[floor(($this->phase + 0.0625) * 8)];
	}
	public function phase_name_FR()
	{
		$nameFR = array('Nouvelle Lune', 'Premier Croissant', 'Premier Quartier', 'Lune Gibbeuse Ascendante', 'Pleine Lune', 'Lune Gibbeuse Descendante', 'Dernier Quartier', 'Dernier Croissant', 'Nouvelle Lune');

		return $nameFR[floor(($this->phase + 0.0625) * 8)];
	}



	public function img_phase()
	{

		$moon_light = round($this->illumination() * 100, 1);
		$get_phase = $this->phase();
		if ($moon_light < 2) {
			$moon_img = 'moon_00';
		} elseif ((($moon_light >= 2) && ($moon_light < 5)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_01';
		} elseif ((($moon_light >= 5) && ($moon_light < 10)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_02';
		} elseif ((($moon_light >= 10) && ($moon_light < 18)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_03';
		} elseif ((($moon_light >= 18) && ($moon_light < 28)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_04';
		} elseif ((($moon_light >= 28) && ($moon_light < 37)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_05';
		} elseif ((($moon_light >= 37) && ($moon_light < 46)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_06';
		} elseif ((($moon_light >= 46) && ($moon_light < 54)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_07';
		} elseif ((($moon_light >= 54) && ($moon_light < 63)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_08';
		} elseif ((($moon_light >= 63) && ($moon_light < 72)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_09';
		} elseif ((($moon_light >= 72) && ($moon_light < 82)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_10';
		} elseif ((($moon_light >= 82) && ($moon_light < 90)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_11';
		} elseif ((($moon_light >= 90) && ($moon_light < 95)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_12';
		} elseif ((($moon_light >= 95) && ($moon_light < 98)) && ($get_phase < 0.5)) {
			$moon_img = 'moon_13';
		} elseif (($moon_light >= 98) && (($get_phase > 0.4) && ($get_phase < 0.6))) {
			$moon_img = 'moon_14';
		} elseif ((($moon_light >= 95) && ($moon_light < 98)) && ($get_phase > 0.5)) {
			$moon_img = 'moon_15';
		} elseif ((($moon_light >= 90) && ($moon_light < 95)) && ($get_phase > 0.5)) {
